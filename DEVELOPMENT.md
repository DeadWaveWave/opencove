...
-   **禁止提交**：
    ...
    -   **调研/验收报告默认不入库**：调研分析报告，以及某次 PR 的 review / acceptance 记录，都只是**本地分析与决策产物**，不是仓库资产。它们是某一时刻的快照，会很快过期，却没有人维护。除非明确要求，否则不要提交；评审结论写在 PR 描述/评论里。`docs/` 只收**本项目自身的**长期文档：架构、不变量、开发流程、案例。若调研中得出的结论具有长期价值（如一条不变量），把**结论**写进对应的项目文档，而不是提交整份报告。
-   **提交前检查（与 CI 对齐的最低门槛）**：
    ...
    -   **纯文档（`.md`）提交的门禁例外**：当 staged 集合只含 `.md`（例如只改 `CHANGELOG.md` 或本文件）时，`pnpm naming-check:staged` 会输出 `no staged files matched — this gate checked NOTHING and is not a pass.` ；退出码取决于环境变量——`scripts/check-naming-staged.mjs:189` 为 `process.exit(process.env.OPENCOVE_REQUIRE_STAGED === '1' ? 1 : 0)`，所以裸跑该脚本退 0（仅告警），而 `OPENCOVE_REQUIRE_STAGED=1 pnpm pre-commit` 会退 1 并中断整条链。因为它的扩展名白名单只含代码类后缀（`.ts/.tsx/.js/.mjs/.cjs/.mts/.cts/.json/.yml/.yaml`），不含 `.md`。这是**预期行为，不是缺陷**：对 0 个文件运行的 gate 本就不该报告通过。此时**不要**为了让它变绿而放宽门禁或把 `.md` 加进白名单——实测把 `.md` 加入白名单会让本文件第 35 行（准确描述历史 `cove:m0:*` 前缀、但不含 `legacy` 豁免词的句子）被判违规。正确做法：如实记录该 gate 未检查任何文件，并以**紧邻的上一次完整产品门禁**作为该提交的运行时证据；链序为 `line-check`(1) → `secret-check`(2) → `naming-check`(3) → … → `format-check`(6)：实测前两道会通过，`naming-check` 在第 3 步以退出码 1 中断整条链，因此 `format-check` 及其后各门**根本不会执行**——不要声称它们通过；需要它们的结论时，单独运行（例如 `npx prettier --check <file>`）。
