...
import { createTerminalDomTextOverhangGeometryCommitScheduler } from '../../../src/contexts/workspace/presentation/renderer/components/terminalNode/syncTerminalNodeSize'
import {
  createDomLayoutContainerMock,
  createTerminalMock,
  installTerminalGeometrySyncWindowMock,
} from './terminalGeometrySync.testUtils'
describe('terminal DOM renderer visual footprint geometry', () => {
  const { ptyResize } = installTerminalGeometrySyncWindowMock()
  it('refreshes DOM renderer visuals after text output exposes overhang without writing PTY geometry', () => {
  ...
  it('does not cascade DOM overhang visual refresh into PTY geometry across repeated frames', () => {
  ...
  it('keeps output-time DOM overhang recovery out of PTY geometry', () => {
    const animationFrames: FrameRequestCallback[] = []
    ;(window as unknown as typeof window).requestAnimationFrame = vi.fn(
      (callback: FrameRequestCallback) => {
        animationFrames.push(callback)
  ...
  it('refreshes DOM renderer visuals when glyph overhang reaches the scrollbar', () => {
    const terminal = createTerminalMock()
    terminal.cols = 107
    terminal.rows = 40
    terminal._core._renderService.dimensions.css.cell = {
  ...
  it('refreshes DOM renderer visual footprint without geometry writes', () => {
    const terminal = createTerminalMock()
    terminal.cols = 110
    terminal.rows = 40
    terminal._core._renderService.dimensions.css.cell = {
