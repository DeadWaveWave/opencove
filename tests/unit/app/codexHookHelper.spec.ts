...
const helperPath = resolve(__dirname, '../../../src/app/cli/hooks/codex-status.mjs')
...
describe('managed Codex hook helper', () => {
  it('exits silently outside an OpenCove-managed Codex spawn', async () => {
    await expect(
      runHelper({
        env: {},
        input: { hook_event_name: 'PermissionRequest', session_id: 'codex-1' },
      }),
    ).resolves.toBe(0)
  })
  ...
  it('posts a normalized authenticated envelope when activated', async () => {
    ...
    try {
      await expect(
        runHelper({
          env: {
            OPENCOVE_CODEX_HOOK_ENDPOINT: `http://127.0.0.1:${address.port}/hooks/codex`,
            OPENCOVE_CODEX_HOOK_TOKEN: 'secret-token',
          },
          input: {
