...
test.describe('Workspace Canvas - Sidebar Row Geometry', () => {
  test('keeps project, space, and agent geometry balanced in docked and rail modes', async () => {
    ...
    try {
      ...
      expect(docked.spaceSurfaceGutterDelta).toBeLessThanOrEqual(1)
      expect(docked.spaceSurfaceCenter).toBeCloseTo(docked.sidebarWidth / 2, 0)
      expect(docked.agentSurfaceProjectLeftGap).toBeGreaterThanOrEqual(5)
      expect(docked.agentSurfaceProjectLeftGap).toBeLessThanOrEqual(7)
