...
const CURRENT_SCHEMA_COLUMNS = {
  ...
  // prettier-ignore
  nodes: ['id', 'workspace_id', 'session_id', 'title', 'title_pinned_by_user', 'position_x', 'position_y', 'width', 'height', 'kind', 'profile_id', 'runtime_kind', 'terminal_geometry_json', 'worker_binding_json', 'terminal_provider_hint', 'label_color_override', 'sidebar_sort_order', 'status', 'started_at', 'ended_at', 'exit_code', 'last_error', 'execution_directory', 'expected_directory', 'agent_json', 'task_json'],
  // prettier-ignore
...
describe('PersistenceStore sort order migration', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(mockDbByPath.get(dbPath)?.userVersion).toBe(13)
      expect(mockDbByPath.get(dbPath)?.workspaceRows).toEqual([
        { id: 'ws-2', sortOrder: 0 },
        { id: 'ws-4', sortOrder: 1 },
        { id: 'ws-1', sortOrder: 2 },
