  ...
  const insertNode = db.prepare(
    `
      INSERT INTO nodes (
        ...
        kind, profile_id, runtime_kind, terminal_geometry_json, worker_binding_json, terminal_provider_hint, label_color_override,
    ...
    for (let sortOrder = 0; sortOrder < state.workspaces.length; sortOrder += 1) {
      ...
      for (const node of workspace.nodes) {
        insertNode.run(
          ...
          node.workerBinding ? safeJsonStringify(node.workerBinding) : null,
